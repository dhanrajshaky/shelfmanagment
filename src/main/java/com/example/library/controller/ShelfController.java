...
public class ShelfController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Shelf> get(@PathVariable Long id) {
        return shelfService.get(id).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }

    @PostMapping
    public Shelf create(@RequestBody Shelf s) { return shelfService.save(s); }

    @PutMapping("/{id}")
    public ResponseEntity<Shelf> update(@PathVariable Long id, @RequestBody Shelf s) {
